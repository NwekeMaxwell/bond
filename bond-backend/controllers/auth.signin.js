...
const { hashPassword, verifyPassword } = require("../services/bcrypt.services");
const { generateToken } = require("../services/jwt.service");
// const randomAvatar = "https://picsum.photos/200/300"
...
const signup = async (req, res) => {
  try {
    ...
    //   const avatar = `<img src="${randomAvatar}" alt="An avatar used to represent ${username} generated with their personal email.">`;
...
const login = async (req, res) => {
  try {
    ...
    if (!email && !username)
      return res
        .status(400)
        .json(`Please enter your email address or username to continue`);
    if (!password)
    ...
    if (foundUser && email && foundUser.deleted === true)
      return res.status(403).json({
        message: `This email is associated with a disabled account, please visit https://bond.onrender.com/users/recover to reactivate your account`,
        success: false,
      });

    if (foundUser && username && foundUser.deleted === true)
      return res.status(403).json({
        message: `This username is associated with a disabled account, please visit https://bond.onrender.com/users/recover to reactivate your account`,
        success: false,
      });
