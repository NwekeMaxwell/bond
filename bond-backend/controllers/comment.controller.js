...
class CommentController{
    ...
    async createComment(req, res) {
        try {
            ...
            const existingPost = await post.find({_id: postid, deleted: false})
            
            if(!existingPost) return res.status(404).json({
                message: `Oops, we couldn't find the post you're trying to reply to as it does not exist or may have been deleted by its author!`,
                success: false
            })

            const newComment = await comment.create({author: existingUser._id, content, parentpost: existingPost._id})
            await newComment.save()

            await post.updateOne({_id: existingPost._id}, newComment._id)
    ...
    async getUserCommentById(req, res) {
        try {
            ...
            const existingComment = await comment.find({_id: id, author: userid, post: postid, deleted: false})
    ...
    async getUserComments(req, res) {
        try {
            ...
            const comments = await comment.findAll({author: userid, post: postid, deleted: false})
